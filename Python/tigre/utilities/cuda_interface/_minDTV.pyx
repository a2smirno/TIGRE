...
cdef extern from "POCS_DTV.hpp":
    cdef void dtv_pocs_tv(float* img, float* dst, float alpha, long* image_size, int maxiter, float delta, c_GpuIds gpuids)


def cuda_raise_errors(error_code):
    if error_code:
        raise ValueError('TIGRE:Call to dtv_pocs_tv failed')


def minDTV(np.ndarray[np.float32_t, ndim=3] src,float alpha = 15.0,int maxiter = 100, float delta=-0.005, gpuids=None):
    ...
    dtv_pocs_tv(c_src, c_imgout, alpha, imgsize, c_maxiter, delta, c_gpuids[0])
